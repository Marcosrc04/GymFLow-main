    ...
    if ($senha != $confirmar_senha) {
        ...
        if ($resultado_verificar->num_rows > 0) {
            ...
            if (isset($_FILES["foto"]) && $_FILES["foto"]["error"] === 0) {
                $pasta_destino = $_SERVER["DOCUMENT_ROOT"] . "/GymFlow-main/arquivos/imagem/perfis/";
                ...
                if (in_array($extensao, $extensoes_permitidas)) {
                    if ($tamanho <= 2 * 1024 * 1024) {
                        ...
                        if (!move_uploaded_file($tmp_nome, $caminho_final)) {
                            die("Erro ao salvar a imagem. Caminho: " . $caminho_final);
                        }
                    } else {
            ...
            $sql = "INSERT INTO usuarios (nome, email, senha, tipo, foto)
                    VALUES ('$nome', '$email', '$senha_hash', 'aluno', " . ($foto_nome_final ? "'$foto_nome_final'" : "NULL") . ")";

            if ($conexao->query($sql) === TRUE) {
                session_start();
                $_SESSION["foto"] = $foto_nome_final;
...
<body>
        ...
        <div class="card-login">
            ...
            <form action="cadastro.php" class="form-login" method="post" enctype="multipart/form-data">
                ...
                <div class="campo">
                    ...
                    <input type="password" id="senha" name="senha" placeholder="Crie uma senha (mín. 8 caracteres)" minlength="8" required>
                </div>
                <div class="campo">
    ...
    <script>
        ...
        inputFoto.addEventListener("change", function() {
            nomeArquivo.textContent = this.files.length > 0 ? this.files[0].name : "Nenhum arquivo escolhido";
        });
    </script>
</body>
</html>